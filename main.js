if (process.argv.includes('--spawned')) {
    ...
    const init_bot = async () => {
        ...
        bot.on('message', async function (jsonMsg) {
			...
			if (textMessage.includes("Summoned to server102 by CONSOLE")) {
				Logger.debug("偵測到召喚至 server102，自動執行 /csafe")
				await chat(bot, "/csafe")
			}
        ...
        bot.on('chat:command', async (messagestr) => {
            ...
            const { getPoints } = require('./utils/points.js');

            // 支援多種點數查詢說法
            const queryKeywords = ['點數', '查點數', '查詢點數', '查看點數', '查點', '幾點'];
            if (queryKeywords.includes(args.trim())) {
                const points = getPoints(playerid);
                await chat(bot, `/m ${playerid} 你目前擁有 ${points} 點。`);
                return;
            }
