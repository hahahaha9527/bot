...
async function process_bet_result(bot, wool, amount, player_id, type, task_uuid) {
    ...
    if (wool == 'yes') {
        ...
        if (type == 'emerald') {
			...
			// 計算點數：每下注1000萬未中，給1點
			const point = Math.floor(amount / 10000000);
			if (point > 0) {
				const { addPoints } = require('../utils/points.js');
				addPoints(player_id, point);
				await chat(bot, `/tell ${player_id} 很可惜你沒中獎，但獲得了 ${point} 點作為回饋！`);
				Logger.log(`玩家 ${player_id} 未中獎，獲得 ${point} 點`);
			}
